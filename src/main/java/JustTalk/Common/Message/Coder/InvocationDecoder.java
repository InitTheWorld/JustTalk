...
public class InvocationDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
        ...
        // 解析成 Invocation
        MessageStruct messageStruct = JSON.parseObject(content, MessageStruct.class);
        out.add(messageStruct);
        logger.info("[decode][连接({}) 解析到一条消息({})]", ctx.channel().id(), messageStruct.toString());
    }
}
